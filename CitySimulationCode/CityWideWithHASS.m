...
for z=1:700 %edit this for part by part simulation (z represents the household numbers to be simulated)
    %tic
    tStart=tic;
    ...
    for a=1:24
        if ((a>=7)&&(a<=11))
            PV(a,1) = 320; %assumed solar module rating
        end
    end
    ...
    for k=1:24 % Find total consumption from utility
        if PV(k,1)>0
            orig_total(1,k) = orig_total(1,k)-PV(k,1);
        end
        if orig_batt_op(1,k) < 0
            orig_total(1,k) = orig_total(1,k)+orig_batt_op(1,k);
    ...
    orig_excess = transpose(PV)-orig_appenergy-orig_batt_op_charging; %computation of PV excess energy
    for k=1:24
    ...
    orig_cost = sum(transpose(orig_total.*price(price_code,:)),1)-sum(transpose(orig_excess.*price(price_code,:))); %computation of final cost for conumer

    fprintf('original cost=%d, peak=%d, and PAR=%d\n', orig_cost, max(orig_total), max(orig_total)/mean(orig_total));
    ...
    simulations=5;
    max_simulations=20; %Poserio: max_sims=5; Our Project: max_sims=20;
    ...
    %count the number of invalid households
    if validctr==0
    ...
    final_cost = sum(transpose(total.*price(price_code,:)),1)-sum(transpose(excess.*price(price_code,:))); %computation of final cost for all consumers in the simulation
    final_peak = max(total); %city simulation peak demand
    final_PAR = final_peak/mean(total); %city simulation PAR
    
    fprintf('cost=%d, peak=%d, and PAR=%d\n', final_cost, final_peak, final_PAR);
    %toc;
    %fprintf('\n');
    ...
    tEnd = tEnd+toc(tStart); %for getting computation time per simulation
   
end

%getting cost for each city-wide simulation cases
...
tAve = tEnd/10; %average computation time for 10 households simulation

fprintf('\n');
fprintf('No of Invalid Schedules:%d, Average Time:%d, and Total Cost:%d \n',num_of_invalid,tAve,city_cost);

%save('UI14_9701-9800.mat') %for sa ving the workspace
%% Plotting
% x = linspace(1,24,24);
