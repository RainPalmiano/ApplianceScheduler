function [OF,satisfaction] = objFunc1(n, t, sched, price, usage, tw, dur, tA, tB, budget, peak, mu, ev_op, batt_op, PV,pR)
...
ev_charge_rate = diff([ev_op ev_op(1,24)]); %limited to charging operation, value >= 0
...
for a=1:n % do for all n appliances
    ...
    if isempty(t_fin(a,:)) || isempty(t_strt(a,:))
        ...
        for b=1:usage(a) % do for all usage times
            ...
            if (t_fin(a,b)<=tB(a,b)&& t_fin(a,b)>=(tA(a,b)+dur(a,b)-1))
                %s_n=((t_fin(a,b))-(tB(a,b)+1))/((tA(a,b)+(dur(a,b)-1))-(tB(a,b)+1)); %Without appliance priority
                s_n=((t_fin(a,b))*((n-pR(a))/(n-1))-(tB(a,b)+1))/((tA(a,b)+(dur(a,b)-1))*((n-pR(a))/(n-1))-(tB(a,b)+1)); %With Appliance priority
            else
    ...
    S_ave=S/usage(a); %obtaining average satisfaction for each usage of specific appliance
    end
TDS=TDS+(1-S_ave)+out_of_bounds; %total dissatisfaction
S_tot=S_tot+S_ave; %total satisfaction for all appliances
end

TDS=TDS*P_dissat*1*Mtds;
S_net=S_tot/n; %net average satisfaction of all appliance of the household
